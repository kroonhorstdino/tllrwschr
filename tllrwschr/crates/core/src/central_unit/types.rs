...
pub(crate) struct UnitCommunicationHandler<ToUnit: FromCommMsgContent, ToDevice: FromUnitMsgContent>
{
    ...
    pub unit_receiver: Option<Mutex<Receiver<FromCommMsg<ToUnit>>>>,
}
...
impl UnitGameOutputSenderHandle {
    pub fn send_output(&self, output: &[GameOutput]) -> anyhow::Result<()> {
        self.send_output_internal(output)
    }

    fn send_output_internal(&self, output: &[GameOutput]) -> anyhow::Result<()> {
        for o in output {
            match o.target {
                OutputTarget::All => {}
